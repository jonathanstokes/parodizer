...
export class Top40Service {
    ...
    async findSongStats(songTitle: string, artist?: string): Promise<Song | null> {
        ...
        for (const song of await Top40Service.getSongCache()) {
            if (songTitle === song.title || stringSimilarity.compareTwoStrings(songTitle, song.title) > Top40Service.TITLE_SIMILARITY_MATCH) {
                if (!artist || artist === song.artist || stringSimilarity.compareTwoStrings(artist, song.artist) > Top40Service.ARTIST_SIMILARITY_MATCH) {
                    if (songTitle !== song.title || artist !== song.artist) {
                        console.warn(`Matched '${songTitle}' by '${artist}' using similarity to '${song.title}' by '${song.artist}'.`);
                    }
